export type Expert = {
  id: string; name: string; role: string; expertise: string[];
  region: string; city: string; country: string;
  latitude: number; longitude: number; email?: string; phone?: string;
};
export type ViewBounds = { west: number; east: number; south: number; north: number };
export function validateProfiles(entries: Record<string, unknown>): Expert[] {
  ...
  return Object.entries(entries).map(([file, value]) => {
    ...
    for (const field of ["id", "name", "role", "region", "city", "country"]) {
      if (typeof p[field] !== "string" || !(p[field] as string).trim()) fail(`${field} must be a nonempty string`);
    }
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(p.id as string)) fail("id must be a lowercase hyphenated identifier");
    if (ids.has(p.id as string)) fail(`duplicate id: ${p.id}`);
    ...
    for (const [key, limit] of [["latitude", 90], ["longitude", 180]] as const) {
      if (typeof p[key] !== "number" || !Number.isFinite(p[key]) || Math.abs(p[key] as number) > limit) fail(`${key} must be a number between -${limit} and ${limit}`);
    }
...
export function filterExperts(experts: Expert[], expertise: string, region: string) {
  return experts.filter(p => (expertise === "all" || p.expertise.includes(expertise)) && (region === "all" || p.region === region));
}
export function inBounds(expert: Expert, bounds: ViewBounds) {
...
export function fitExtent(experts: Expert[]): [[number, number], [number, number]] | null {
